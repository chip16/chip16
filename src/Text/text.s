...
;-------------------------------------------------------------------------------
; Program entry point
_start:			call store_ptrs
				bgc 0x0					; Black bg
				ldi rb, 0				; X = 0
				ldi rc, 0				; Y = 0
print_str:		rnd ra, MAX_STR_IND		; Get a random string
...
get_input:		ldi ra, 15				; 15 VBLNKs = 0.25s
...
; [RA: 		Char ptr]
; [RB, C: 	X, Y]
text_disp:		spr CHAR_SPR_SIZE			; Set sprite size
				push r0						; Preserve R0
text_disp_loop:	ldm r0, ra					; Load 2 bytes from memory
...
text_disp_next:	addi rb, CHAR_SPR_WIDTH		; Move X coordinate along
				cmpi rb, SCR_WIDTH			
				jl text_disp_itp			; If x >= 320...
				ldi rb, 0					; ...wrap back to 0...
				addi rc, 8					; ...and go to new line
text_disp_itp:	addi ra, 1					; Increment text pointer
				jmp text_disp_loop			; Repeat...
text_disp_end:	pop r0						; Restore R0
				ret

; Pauses the program
; [RA:	number of VBLNKs to wait]
pause_prog:		cmpi ra, 0					; Check number of VBLNKs left
...
;-------------------------------------------------------------------------------
; Our test strings
string1:	db "Hello, this is a string printing test. Text should wrap..."
			db 0
string2:	db "So, here is another string... Making these long so they wrap!"
			db 0
string3:	db "Can you guess what font this is? It's from an 80's computer..."
			db 0
string4:	db "It's the eeey-eeey-eeeey-eeey-eeeey-eeye... of the tiger"
			db 0
; Pointers to these strings
strptrs:	db 0x00, 0x00
			db 0x00, 0x00
			db 0x00, 0x00
			db 0x00, 0x00